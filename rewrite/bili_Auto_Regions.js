/**
from:https://github.com/NobyDa/Script/blob/master/Surge/JS/Bili_Auto_Regions.js
**/
let $ = nobyda();
let run = EnvInfo();

async function SwitchRegion(play) {
	const Group = $.read('港台番剧'); //Your blibli policy group name.
	const CN = 'DIRECT'; //Your China sub-policy name.
	const TW = $.read('TW'); //Your Taiwan sub-policy name.
	const HK = $.read('HK'); //Your HongKong sub-policy name.
	const DF = 'HK'; //Sub-policy name used after region is blocked(e.g. url 404)
